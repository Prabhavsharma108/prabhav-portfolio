...
// Analytics is opt-in and loaded lazily, so visitors who never get one don't
// pay for the script. Without a key configured it is never fetched at all.
...
const GlobalAudioEnabler = () => {
  ...
  useEffect(() => {
    const handleInteraction = () => {
      // First gesture: only now is it worth paying for the audio download.
      initAudio();
      enableAudio();
    };
...
function AppContent() {
  ...
  // Audio is deliberately NOT initialised here. Browsers block playback until
  // a user gesture anyway, so fetching the track on mount only steals
  // bandwidth from the textures the first frame actually needs.
  // GlobalAudioEnabler arms it on the first interaction instead.
          ...
          <div className="canvas-wrapper">
            <Canvas
              ...
              gl={{
                ...
                // Deliberately NOT failIfMajorPerformanceCaveat. On a machine
                // with no hardware acceleration that flag makes context
                // creation fail outright and the visitor gets a blank page.
                // The PerformanceMonitor below already drops quality when
                // frames are slow, so degrading beats not rendering at all.
              }}
              dpr={settings.dpr}
