...
const MonitorBlock = memo(({ item, meshRef, isSelected, onMonitorClick, disabled, paintOnBeforeCompile, paintUniforms }) => {
    ...
    const isPhoneMonitor = shape === 'phone';

    // Determine the URL for the front texture (custom or default)
    const frontTextureUrl = item.frontTexture || (
        isBlogMonitor ? '/textures/studio/monitorfront_postnafbdoublewinner.webp' :
            isTvMonitor ? '/textures/studio/tvfront_filmikprojektdlamultiego.webp' :
    ...
    const paintedFrontTextureUrl = isTouch ? dummyTex : (item.paintedFrontTexture || (
        isBlogMonitor ? '/textures/studio/monitorfront_postnafbdoublewinner_painted.webp' :
            isTvMonitor ? '/textures/studio/tvfront_filmikprojektdlamultiego_painted.webp' :
