...
const WorkExperienceMilestone = ({ z, scrollProgressRef }) => {
    ...
    const groupRef = useRef();
    const hirequotientRef = useRef();
    const subhsandeshRef = useRef();
    const drawflowRef = useRef();

    // Card reveal refs (driven by button hover)
    const subhsandeshCardRevealRef = useRef();
    const subhsandeshCardPaintedRef = useRef();
    const subhsandeshHideDelayRef = useRef();
    const drawflowCardRevealRef = useRef();
    const drawflowCardPaintedRef = useRef();
    const drawflowHideDelayRef = useRef();
    const hirequotientCardRevealRef = useRef();
    const hirequotientCardPaintedRef = useRef();
    const hirequotientHideDelayRef = useRef();

    // Load textures - using same card template for all companies
    const hirequotientTexture = useLoader(THREE.TextureLoader, '/textures/about/SOTY.webp');
    const subhsandeshTexture = useLoader(THREE.TextureLoader, '/textures/about/SOTD.webp');
    const drawflowTexture = useLoader(THREE.TextureLoader, '/textures/about/SOTM.webp');
    const hirequotientPaintedTexture = useLoader(THREE.TextureLoader, isTouch ? '/textures/about/SOTY.webp' : '/textures/about/SOTY_painted.webp');
    const subhsandeshPaintedTexture = useLoader(THREE.TextureLoader, isTouch ? '/textures/about/SOTD.webp' : '/textures/about/SOTD_painted.webp');
    const drawflowPaintedTexture = useLoader(THREE.TextureLoader, isTouch ? '/textures/about/SOTM.webp' : '/textures/about/SOTM_painted.webp');
    ...
    // Color space fix
    hirequotientTexture.colorSpace = THREE.SRGBColorSpace;
    subhsandeshTexture.colorSpace = THREE.SRGBColorSpace;
    drawflowTexture.colorSpace = THREE.SRGBColorSpace;
    hirequotientPaintedTexture.colorSpace = THREE.SRGBColorSpace;
    subhsandeshPaintedTexture.colorSpace = THREE.SRGBColorSpace;
    drawflowPaintedTexture.colorSpace = THREE.SRGBColorSpace;
    ...
    useFrame((state) => {
        ...
        if (subhsandeshRef.current) {
            subhsandeshRef.current.position.x = -revealFactor * spreadX;
        }
        if (drawflowRef.current) {
            drawflowRef.current.position.x = revealFactor * spreadX;
        }

        if (hirequotientRef.current) {
            hirequotientRef.current.position.y = 0.5 + sotyFactor * 2.5;
        }
    });
    ...
    return (
        <group ref={groupRef} position={[0, 2, z]}>
            ...
            {/* === SUBHSANDESH (left card, rendered second) === */}
            <group ref={subhsandeshRef} position={[0, 0.5, -0.5]}>
                {/* Painted card (behind) - hidden until button hover */}
                <mesh ref={subhsandeshCardPaintedRef} position={[0, 0, -0.001]} visible={true}>
                    <planeGeometry args={[cardHeight * cardLegacyAspect, cardHeight]} />
                    <meshBasicMaterial color="#e0e0e0"
                        map={subhsandeshPaintedTexture}
                        transparent
                        opacity={0}
                        side={THREE.DoubleSide}
                        alphaTest={0.5}
                ...
                <mesh>
                    <planeGeometry args={[cardHeight * cardLegacyAspect, cardHeight]} />
                    <revealBasicMaterial
                        ref={subhsandeshCardRevealRef}
                        map={subhsandeshTexture}
                        transparent
                        side={THREE.DoubleSide}
                        uProgress={0.0}
                    />
            ...
            {/* === DRAWFLOW (right card, rendered third) === */}
            <group ref={drawflowRef} position={[0, 0.5, -0.2]}>
                {/* Painted card (behind) - hidden until button hover */}
                <mesh ref={drawflowCardPaintedRef} position={[0, 0, -0.001]} visible={true}>
                    <planeGeometry args={[cardHeight * cardLegacyAspect, cardHeight]} />
                    <meshBasicMaterial color="#e0e0e0"
                        map={drawflowPaintedTexture}
                        transparent
                        opacity={0}
                        side={THREE.DoubleSide}
                        alphaTest={0.5}
                ...
                <AwardButton
                    ...
                    height={buttonHeight}
                    position={[0, buttonY, 0.05]}
                    onHoverChange={makeCardHoverHandler(drawflowCardRevealRef, drawflowCardPaintedRef, drawflowHideDelayRef)}
                />
                {/* COMPANY NAME */}
                <Text
                    position={[0, 0.7, 0.01]}
            ...
            {/* === HIREQUOTIENT (front, center, rendered LAST = always on top) === */}
            <group ref={hirequotientRef} position={[0, 0.5, 0]}>
                {/* Painted card (behind) - hidden until button hover */}
                <mesh ref={hirequotientCardPaintedRef} position={[0, 0, -0.001]} visible={true}>
                    <planeGeometry args={[cardHeight * cardLegacyAspect, cardHeight]} />
                    <meshBasicMaterial color="#e0e0e0"
                        map={hirequotientPaintedTexture}
                        transparent
                        opacity={0}
                        side={THREE.DoubleSide}
                        alphaTest={0.5}
                ...
                <mesh>
                    <planeGeometry args={[cardHeight * cardLegacyAspect, cardHeight]} />
                    <revealBasicMaterial
                        ref={hirequotientCardRevealRef}
                        map={hirequotientTexture}
                        transparent
                        side={THREE.DoubleSide}
                    />
                </mesh>
                ...
                <AwardButton
                    onClick={(e) => {
                        e.stopPropagation();
                        openOverlay(WORK_EXPERIENCE_DATA.hirequotient);
                    }}
                    texture={buttonTexture}
                    paintedTexture={buttonPaintedTexture}
                    width={buttonWidth}
                    height={buttonHeight}
                    position={[0, buttonY, 0.05]}
                    onHoverChange={makeCardHoverHandler(hirequotientCardRevealRef, hirequotientCardPaintedRef, hirequotientHideDelayRef)}
                />
                {/* COMPANY NAME */}
                <Text
                    position={[0, 0.7, 0.01]}
